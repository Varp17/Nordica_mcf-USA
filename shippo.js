...
console.log("🔍 SHIPPO_API_TOKEN loaded:", process.env.SHIPPO_API_TOKEN ? "YES" : "NO (Check .env)");
  ...
  async (req, res) => {
    try {
      ...
      const [orders] = await db.execute(
        `SELECT
           o.id, o.created_at, o.status, o.payment_status, o.total,
           o.shipping_address,
    ...
    try {
      ...
      if (!history.length) {
        try {
          ...
          console.error("âš  Live tracking refresh failed:", apiErr.message);
        }
      }
...
router.get("/trackings", authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { status, page = 1 } = req.query;

    console.log("📊 GET /trackings (Shippo live API)");
      ...
      results.map(async (t) => {
        ...
        if (trackingNumber && carrier) {
          try {
            ...
            console.error("⚠️ trackingStatus.get failed for", carrier, trackingNumber, err.message);
          }
        }
    ...
    res.status(500).json({ success: false, error: "Failed to list trackings" });
  }
});
...
router.get("/transactions", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    console.log("🏷️ GET /transactions (Shippo live API)");
...
router.get("/orders", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    console.log("📦 GET /orders (Shippo live API)");
    ...
    console.error("List shippo orders error:", error);
    res.status(500).json({ success: false, error: "Failed to list shippo orders" });
  }
});
