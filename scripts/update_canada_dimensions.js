...
const productData = [
  {
    ...
    net_g: 410, gross_g: 410,
    length_in: 4.4, width_in: 4, height_in: 5.4
  },
  {
    name: 'Pop Color Mitt',
    matchNames: ['ULTRA SOFT COLOR-POP WASH MITT'],
    net_g: 76.5, gross_g: 76.5,
    length_in: 11, width_in: 9.2, height_in: 0.8
  },
  {
  ...
  },
  {
    ...
    net_g: 490, gross_g: 490,
    length_in: 10.70, width_in: 2.75, height_in: 10.70,
  ...
  },
  {
    ...
    net_g: 400, gross_g: 400,
    length_in: 8.5, width_in: 10, height_in: 2,
  ...
  },
  {
    ...
    net_g: 250, gross_g: 250,
    length_in: 6.75, width_in: 2.75, height_in: 4.5,
    variants: ['Black', 'Blue', 'Red', 'Yellow', 'Neon Green']
  },
  {
    name: 'DG-5GAL',
    matchNames: ['DETAIL GUARDZ 5 GALLON DETAILING BUCKET'],
    net_g: 750, gross_g: 750,
    length_in: 12, width_in: 12, height_in: 14.5
  },
  {
    name: 'Dirt Lock-SAP',
    ...
    net_g: 250, gross_g: 250,
    length_in: 7.5, width_in: 7.5, height_in: 4,
    variants: ['Black', 'White']
  },
  {
    name: 'Dirt Lock-PWS',
  ...
  },
  {
    ...
    net_g: 700, gross_g: 700,
    length_in: 2.25, width_in: 4.25, height_in: 10.25
  }
];

async function updateCanadaDimensions() {
  ...
  console.log('Connected to database. Starting updates exclusively for Canada variants...');
  ...
  for (const data of productData) {
    ...
    for (const matchName of data.matchNames) {
      ...
      // 1. Update the base product ONLY if target_country is 'canada' or 'both'
      const [updateRes] = await db.execute(
        `UPDATE products SET 
          ...
          dimensions = ?, 
          dimensions_imperial = ? 
        WHERE name = ? AND target_country IN ('canada', 'both')`,
        [weight_kg, weight_lb, dimensions, dimensions_imperial, matchName]
      );

      if (updateRes.affectedRows > 0) {
        console.log(`  Updated base product (Canada/Both): ${matchName}`);
      }

      // 2. Handle variants in color_options JSON ONLY if the product is Canadian targeted
      const [products] = await db.execute(
        'SELECT id, color_options FROM products WHERE name = ? AND target_country IN (\'canada\', \'both\')',
        [matchName]
      );
      ...
      // 3. Update product_variants table explicitly for Canadian variants
      try {
        const [pvUpdate] = await db.execute(
          `UPDATE product_variants v
           JOIN products p ON v.product_id = p.id
           SET v.weight_kg = ?, v.weight_lb = ?, v.dimensions = ?, v.dimensions_imperial = ?
           WHERE p.name = ? AND v.target_country IN ('canada', 'both')`,
          [weight_kg, weight_lb, dimensions, dimensions_imperial, matchName]
        );
      ...
      // 4. Update product_color_variants table explicitly for Canadian variants
      try {
        const [pcvUpdate] = await db.execute(
          `UPDATE product_color_variants cv
           ...
           WHERE p.name = ? AND cv.target_country IN ('canada', 'both')`,
          [weight_kg, weight_lb, dimensions, dimensions_imperial, matchName]
        );
  ...
  console.log('All Canadian targeted updates completed.');
  await db.end();
}

updateCanadaDimensions().catch(err => {
